...
- 最佳 learned-model 方法是 **Dyna empirical (10)**，tail exact gain 为 `2.1906 ± 0.0084`（95% CI），为 oracle 的 `91.7%`。
- Dyna empirical(10) 相对 Q-learning 的 paired exact-gain 差为 `0.1556 ± 0.0291`；95% CI不跨过 0。
- Dyna empirical(10) 相对 Q-learning 的全程 exact-gain AUC paired 差为 `0.1341 ± 0.0538`。
- 达到 80% oracle gain 的步数差（Dyna−Q）为 `830 ± 470`；负值代表 Dyna 更快。
- 等 update-budget 的 Replay-Q 相对 Q-learning 差为 `-0.0454 ± 0.2109`，用于区分 model structure 与单纯增加更新次数。
- latest model 的 tail TV error 为 `0.262 ± 0.004`，empirical model 为 `0.086 ± 0.002`；随机 dynamics 下 latest-transition table 存在持续偏差。
