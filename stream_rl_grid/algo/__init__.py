...
from .expected_sarsa import DifferentialExpectedSarsa, DifferentialExpectedSarsaTIDBD
...
def create_agent(features, config, seed=0):
    ...
    if config.algorithm == "expected_sarsa":
        return DifferentialExpectedSarsa(features, config, seed=seed)
    if config.algorithm == "expected_sarsa_tidbd":
        return DifferentialExpectedSarsaTIDBD(features, config, seed=seed)
