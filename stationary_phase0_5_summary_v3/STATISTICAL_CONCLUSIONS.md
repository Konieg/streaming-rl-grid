...
- 最佳 learned-model 方法是 **Dyna empirical (10)**，tail exact gain 为 `2.0574 ± 0.0676`（95% CI），为 oracle 的 `86.1%`。
- Dyna empirical(10) 相对 Q-learning 的 paired exact-gain 差为 `0.0924 ± 0.1182`；95% CI跨过 0。
- Dyna empirical(10) 相对 Q-learning 的全程 exact-gain AUC paired 差为 `0.0755 ± 0.0474`。
- 达到 80% oracle gain 的步数差（Dyna−Q）为 `-667 ± 509`；负值代表 Dyna 更快。
- 等 update-budget 的 Replay-Q 相对 Q-learning 差为 `0.0153 ± 0.2415`，用于区分 model structure 与单纯增加更新次数。
- latest model 的 tail TV error 为 `0.260 ± 0.006`，empirical model 为 `0.092 ± 0.003`；随机 dynamics 下 latest-transition table 存在持续偏差。
