...
- 最佳 learned-model 方法是 **Dyna empirical (5)**，tail exact gain 为 `2.0368 ± 0.0266`（95% CI），为 oracle 的 `85.2%`。
- Dyna empirical(10) 相对 Q-learning 的 paired exact-gain 差为 `-0.0301 ± 0.0368`；95% CI跨过 0。
- Dyna empirical(10) 相对 Q-learning 的全程 exact-gain AUC paired 差为 `0.0900 ± 0.0272`。
- 达到 80% oracle gain 的步数差（Dyna−Q）为 `-263 ± 278`；负值代表 Dyna 更快。
- 等 update-budget 的 Replay-Q 相对 Q-learning 差为 `-0.0454 ± 0.2109`，用于区分 model structure 与单纯增加更新次数。
- latest model 的 tail TV error 为 `0.261 ± 0.005`，empirical model 为 `0.093 ± 0.003`；随机 dynamics 下 latest-transition table 存在持续偏差。
